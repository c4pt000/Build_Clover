...
# --------------------------------------
SCRIPTVER="v5.1.6-q35-speed-patch-r4920"
#GCC8(.3.0)
export GCCVERS=8
...
RSCRIPT_INFO="sync edk2 svn r28976 (tagged as edk2-stable201112)"
RSCRIPTVER=""
export LC_ALL=C
SYSNAME="$( uname )"
...
getRev() {
REMOTE_REV=$(svnWithErrorCheck "info https://github.com/c4pt000/clover-4920-q35" | grep '^Revision:' | tr -cd [:digit:])
REMOTE_EDK2_REV=$(svnWithErrorCheck "info https://github.com/tianocore/edk2" | grep '^Revision:' | tr -cd [:digit:])
...
if [[ "$ForceEDK2Update" -eq "0" ]]; then
	...
	svnWithErrorCheck "--depth empty co $revision --non-interactive --trust-server-cert https://github.com/tianocore/edk2 ."
...
if [[ -z "$SUGGESTED_CLOVER_REV" ]]; then
	...
	if [[ ! -d "${DIR_MAIN}/edk2/Clover" ]]; then
		...
		if IsNumericOnly "${REMOTE_REV}"; then
			...
			cmd="co -r $REMOTE_REV --non-interactive --trust-server-cert https://github.com/c4pt000/clover-4920-q35 ."
		else
	...
	if [[ ! -d "${DIR_MAIN}/edk2/Clover" ]] ; then
		...
		mkdir -p "${DIR_MAIN}"/edk2/Clover
		cmd="co -r $SUGGESTED_CLOVER_REV --non-interactive --trust-server-cert https://github.com/c4pt000/clover-4920-q35 ."
	else
		if [[ "${LOCAL_REV}" == "" ]]; then
...
TIMES=0
IsLinkOnline https://github.com/c4pt000/clover-4920-q35
cd "${DIR_MAIN}"/edk2/Clover
