...
# --------------------------------------
SCRIPTVER="v5.1.6 with speed patch q35 KVM"
#GCC8(.3.0)
export GCCVERS=5
...
BuildCloverRepo="https://github.com/c4pt000/Build_Clover.git"
...
var_defaults=(
	...
	"USEAPFS",,,"NO"
	"USENTFS",,,"NO"
	"GITHUB",,,"https://raw.githubusercontent.com/c4pt000/Build_Clover/master/Build_Clover-with-speed-patch.sh"
	"CLOVER_REP",,,"https://github.com/c4pt000/clover"
	"EDK2_REP",,,"https://github.com/c4pt000/edk2"
	"DISABLE_CLEAR",,,"NO"
	"MY_SCRIPT",,,
	"FAST_UPDATE",,,"NO"
	"INTERACTIVE",,,"YES"
...
local ghlink="https://github.com/c4pt000/Build_Clover/tree/work/Files"
local efifiles=()
...
printf "\nA new Build_Clover-with-speed-patch.sh is available,\n"
...
case $answer in
	Y | y)
		if [[ -f /tmp/Build_Clover.tmp ]]; then
			...
			pressAnyKey 'Was not possible to update Build_Clover-with-speed-patch.sh,'
		fi;;
esac
}
# --------------------------------------
...
if [[ $? -ne 0 ]] ; then
	...
	echo "to simply run Build_Clover-with-speed-patch.sh.."
...
if [[ "$SYSNAME" == Linux ]]; then
	...
	if [[ "$(uname -m)" != x86_64 ]]; then
		printError "\nBuild_Clover-with-speed-patch.sh is tested only on x86_64 architecture, aborting..\n"
		exit 1
	fi
...
if [[ "$ForceEDK2Update" -eq "0" ]]; then
	...
	svnWithErrorCheck "git clone $EDK2_REP ."
		...
		if [[ "$d" != "Source" && "$d" != "Scripts" ]]; then
			...
			if [[ -d "${DIR_MAIN}/edk2/${d}" ]] ; then
				if [[ -d "${DIR_MAIN}/edk2/${d}/.svn" ]] ; then
					...
					cd "${DIR_MAIN}/edk2/${d}"
					rm -rf ./* > /dev/null 2>&1
					svnWithErrorCheck "git clone $EDK2_REP/${d} ."
				fi
			else
				cd "${DIR_MAIN}"/edk2
				svnWithErrorCheck "git clone $EDK2_REP/${d}"
...
IA32_X64 | X64 )
	...
	local newDesc=$(echo "\"cloverEFI.64.blockio_description\" = \"Built using Build_Clover-with-speed-patch.sh with: ${DEFINED_MACRO}\";")
...
IA32 )
	...
	local newDesc=$(echo "\"cloverEFI.32_description\" = \"Built using Build_Clover-with-speed-patch.sh with: ${DEFINED_MACRO}\";");;
esac
...
if [[ -d "${DIR_MAIN}/edk2/Clover/.svn" && "$INTERACTIVE" != "NO" ]] ; then
	...
	if [[ "$SELF_UPDATE_OPT" == YES ]]; then options+=("update Build_Clover-with-speed-patch.sh"); fi
	...
	do
		case $opt in
			"update Build_Clover-with-speed-patch.sh" \
	...
	case $choice in
		...
		"update Build_Clover-with-speed-patch.sh" ) selfUpdate; cbuild;;
